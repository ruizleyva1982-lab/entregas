...
if df is not None:
    ...
    if not columnas_existentes:
        st.error("No se encontraron las columnas. Verifica los nombres en tu archivo.")
        ...
        if filtro_articulo and "Número de artículo" in df.columns:
            df_filtrado = df_filtrado[df_filtrado["Número de artículo"].str.contains(filtro_articulo, case=False, na=False)]
        if filtro_descripcion and "Descripción del artículo" in df.columns:
            df_filtrado = df_filtrado[df_filtrado["Descripción del artículo"].str.contains(filtro_descripcion, case=False, na=False)]
        if fecha_min and "Fecha de vencimiento" in df.columns:
            df_filtrado["Fecha de vencimiento"] = pd.to_datetime(df_filtrado["Fecha de vencimiento"], errors="coerce")
            df_filtrado = df_filtrado[df_filtrado["Fecha de vencimiento"] >= pd.Timestamp(fecha_min)]
        if fecha_max and "Fecha de vencimiento" in df.columns:
            df_filtrado["Fecha de vencimiento"] = pd.to_datetime(df_filtrado["Fecha de vencimiento"], errors="coerce")
